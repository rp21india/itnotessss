...
<%
  String query = "select university.uname,course.course,yearsem.yearsem,subject.sname,notes.notes_id,notes.filename from university join course on university.uid=course.uid join yearsem on course.cid=yearsem.cid join subject on yearsem.yid=subject.yid join notes on subject.sid=notes.sid where notes.sid="+request.getParameter("sid");
  ...
  while(rs.next())
  {
    out.println("<tr>"
        ...
        + "<td>"
        +   "<a href='downloadnotes.jsp?notes_id="+rs.getInt("notes_id") +"'> Download </a>"
        + "</td>"
        + "</tr>");
    count++;
  }
